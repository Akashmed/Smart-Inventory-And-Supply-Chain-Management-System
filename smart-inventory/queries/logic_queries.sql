...
-- Procedure for inbound shipments
...
AS
BEGIN
    -- Insert inbound shipment record
    INSERT INTO supplier_shipments (supplier_id, product_id, quantity, received_by)
    VALUES (p_supplier_id, p_product_id, p_quantity, p_received_by);

    -- Insert inbound inventory log
    INSERT INTO inventory_logs (product_id, change_type, quantity_changed, remarks)
    VALUES (p_product_id, 'inbound', p_quantity, 'Restocked via supplier shipment');
...
FOR EACH ROW
BEGIN
    INSERT INTO shipments (order_id, shipped_date, carrier, tracking_number, product_id)
    VALUES (:NEW.order_id, SYSDATE, 'DefaultCarrier', 'TRK-' || :NEW.order_id, :NEW.product_id);
END;
/
